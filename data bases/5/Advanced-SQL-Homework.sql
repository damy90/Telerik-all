...
--4
SELECT AVG(Salary)  [Average Salary Department #1]
FROM Employees
WHERE DepartmentID = 1

--5
SELECT AVG(Salary) [Average Salary in Sales]
FROM Employees e
...
--6
SELECT COUNT(EmployeeID) [Number of employees in Sales]
FROM Employees e
WHERE e.DepartmentID = 
...
SELECT e.EmployeeID, e.FirstName + ' ' + e.LastName AS Manager
FROM Employees e
JOIN Employees m
...
--"day.month.year hour:minutes:seconds:milliseconds". Search in  Google to find how
--to format dates in SQL Server.
SELECT FORMAT ( GETDATE(), 'dd.MM.yyyy hh:mm:ss:mmm', 'en-US' ) AS DateConvert

--15. Write a SQL statement to create a table Users. Users should have username, password, full
