...
function buildDictionary(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        dictionary[mother] = deepCopy(data[i]);
    ...
    for (name in dictionary) {
        if (name.substring(name.length - 2, name.length) === 'va') {
            dictionary[name].isFemale = true;
        }
    }
    return dictionary;
}
...
function findRoot(tree, dictionary) {
    ...
    for (var i = 0; i < tree.length; i++) {
        var mother = deepCopy(tree[i].mother);
        var father = deepCopy(tree[i].father);
        var isRoot = true;
